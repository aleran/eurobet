<?php
	...
	if (isset($_GET["anular"])) {
		$sql1="SELECT cedula, monto FROM tickets_c WHERE codigo='".$_GET["anular"]."'";
		...
		$sql_anular="UPDATE tickets_c SET activo='0' WHERE codigo='".$_GET["anular"]."'";
		$rs_anular=mysqli_query($mysqli,$sql_anular) or die(mysqli_error($mysqli));
		echo "<script>alert('ticket anulado');window.location='activos_c.php?desde=".$_GET["desde"]."&hasta=".$_GET["hasta"]."'</script>";
	}

	if (isset($_GET["activar"])) {
		$sql1="SELECT cedula, monto FROM tickets_c WHERE codigo='".$_GET["activar"]."'";
		$rs1=mysqli_query($mysqli,$sql1) or die(mysqli_error());
		$row1=mysqli_fetch_array($rs1);
		...
		$sql_anular="UPDATE tickets_c SET activo='1' WHERE codigo='".$_GET["activar"]."'";
		$rs_anular=mysqli_query($mysqli,$sql_anular) or die(mysqli_error($mysqli));
		echo "<script>alert('ticket Activado');window.location='anulados_c.php?desde=".$_GET["desde"]."&hasta=".$_GET["hasta"]."'</script>";
	}

	if (isset($_GET["ganar"])) {

		$sql_premio="SELECT premio, cedula FROM tickets_c WHERE codigo='".$_GET["ganar"]."'";
		...
		if ($row_premio["cedula"]=="") {
			$sql_ganar="UPDATE tickets_c SET ganar='1' WHERE codigo='".$_GET["ganar"]."'";
			$rs_ganar=mysqli_query($mysqli,$sql_ganar) or die(mysqli_error($mysqli));
			echo "<script>alert('ticket marcado como Ganador');window.location='activos_c.php?desde=".$_GET["desde"]."&hasta=".$_GET["hasta"]."'</script>";
	...
	if (isset($_GET["perder"])) {
		...
		$sql_perder="UPDATE tickets_c SET ganar='0' WHERE codigo='".$_GET["perder"]."'";
		$rs_perder=mysqli_query($mysqli,$sql_perder) or die(mysqli_error($mysqli));
		echo "<script>alert('ticket marcado como Perdedor');window.location='activos_c.php?desde=".$_GET["desde"]."&hasta=".$_GET["hasta"]."'</script>";
	}
	if (isset($_GET["pagar"])) {

		$sql_perder="UPDATE tickets_c SET pagado='1' WHERE codigo='".$_GET["pagar"]."'";
		$rs_perder=mysqli_query($mysqli,$sql_perder) or die(mysqli_error($mysqli));
		echo "<script>alert('Ticket Pagado');window.location='por_pagar_c.php'</script>";
	}
?>
