                  ...
                  <h3><center><b>Premios pendientes por Pagar</b></center></h3><br>
	                ...
	                <?php
	                	...
	                	if ($_SESSION["tipo"]=="root") {
	                		$sql_act="SELECT * FROM tickets_c WHERE activo='1' AND ganar='1' AND cedula='' AND pagado='0'";
                            $sql_t_pagar="SELECT SUM(premio) AS t_pagar FROM tickets_c WHERE activo='1' AND ganar='1' AND cedula='' AND pagado='0'";
                            $rs_t_pagar=mysqli_query($mysqli, $sql_t_pagar) or die(mysqli_error());
                            $row_t_pagar=mysqli_fetch_array($rs_t_pagar);
	                	}
	                	else {
	                		$sql_act="SELECT * FROM tickets_c WHERE activo='1' AND ganar='1' AND cedula='' AND pagado='0' AND agencia='".$_SESSION["agencia"]."'";

                            $sql_t_pagar="SELECT SUM(premio) AS t_pagar FROM tickets_c WHERE activo='1' AND ganar='1' AND cedula='' AND pagado='0' AND agencia='".$_SESSION["agencia"]."'";
                            $rs_t_pagar=mysqli_query($mysqli, $sql_t_pagar) or die(mysqli_error());
                            $row_t_pagar=mysqli_fetch_array($rs_t_pagar);
	                	}
	                    ...
	                    while ($row_act=mysqli_fetch_array($rs_act)) {
	                    		...
	                    		echo"<tr>";
	                    			echo"<td>";
	                    				echo "<a href='t_pagar.php?codigo=".$row_act["codigo"]."'>".$row_act["codigo"]."</a>";
	                    			echo"</td>";
	                    			echo"<td>";
	                    				echo $row_act["tipo"];
	                    			echo"</td>";
