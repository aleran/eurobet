                ...
                <div class="row">
	                ...
	                <?php
	                	...
	                	if ($_SESSION["tipo"]=="root") {
	                		$sql_act="SELECT * FROM tickets_c WHERE activo='1' AND ganar='3' AND pagado='0' AND (fecha BETWEEN '".$desde."' AND '".$hasta."')";

                            $sql_t_apostado="SELECT SUM(monto) AS t_apostado FROM tickets_c WHERE activo='1' AND ganar='3' AND (fecha BETWEEN '".$desde."' AND '".$hasta."')";
                            $rs_t_apostado=mysqli_query($mysqli, $sql_t_apostado) or die(mysqli_error());
                            $row_t_apostado=mysqli_fetch_array($rs_t_apostado);
	                	}
	                	else {
	                		$sql_act="SELECT * FROM tickets_c WHERE activo='1' AND ganar='3' AND pagado='0' AND agencia='".$_SESSION["agencia"]."'AND (fecha BETWEEN '".$desde."' AND '".$hasta."')";

                            $sql_t_apostado="SELECT SUM(monto) AS t_apostado FROM tickets_c WHERE activo='1' AND ganar='3' AND agencia='".$_SESSION["agencia"]."' AND (fecha BETWEEN '".$desde."' AND '".$hasta."')";
                            $rs_t_apostado=mysqli_query($mysqli, $sql_t_apostado) or die(mysqli_error());
                            $row_t_apostado=mysqli_fetch_array($rs_t_apostado);
	                	}
