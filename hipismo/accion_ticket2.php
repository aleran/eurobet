<?php
	...
	if (isset($_GET["anular"])) {
		...
		echo "<script>alert('ticket anulado');window.location='consultas_c.php'</script>";
	}
	...
	if (isset($_GET["perder"])) {
		...
		echo "<script>alert('ticket marcado como Perdedor');window.location='consultas_c.php'</script>";
	}
	if (isset($_GET["pagar"])) {

		$sql_perder="UPDATE tickets_c SET pagado='1' WHERE codigo='".$_GET["pagar"]."'";
		$rs_perder=mysqli_query($mysqli,$sql_perder) or die(mysqli_error($mysqli));
		echo "<script>alert('Ticket Pagado');window.location='consultas_c.php'</script>";
	}
?>